...
class PlotDumpFields(object):
    ...
    def fft_field(self, radius, field_component, colour=1, canvas = None):
        print "Doing fft for", field_component, "...",
        sys.stdout.flush()
        min_1, max_1, n_1 = self.get_bin_list("r")
        bin_size = (max_1 - min_1)/(n_1+1)
        fft_list = []
        for i in range(self.n_lines):
            if abs(radius - self.field_map["r"][i]) > bin_size/2.:
                continue
            fft_list.append((self.field_map["phi"][i], self.field_map[field_component][i]))
        fft_list = sorted(fft_list)
        fft_list = [field[1] for field in fft_list]

        fft = numpy.fft.rfft(fft_list)
        fft_mag = [numpy.absolute(item) for item in fft]
        print fft[:3], "...", fft[-3:]
        print fft_mag[:3], "...", fft_mag[-3:]
        freq = [i*1./len(fft_mag) for i in range(len(fft_mag))]
